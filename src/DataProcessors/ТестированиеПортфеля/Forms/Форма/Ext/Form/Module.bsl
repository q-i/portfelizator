...
Функция ПолучитьДатуСледующейРебалансировки(ОписаниеПортфеля)
	...
	ПредДата = ОписаниеПортфеля.ДатаПредыдущейРебалансировки;
	
	ПараметрыРебалансировки = ОписаниеПортфеля.ПараметрыРебалансировки;
	...
	ИначеЕсли ПараметрыРебалансировки.Периодичность = Перечисления.ПериодичностьРебалансировки.Ежемесячно Тогда
		...
		Если ПараметрыРебалансировки.ВариантВыбораДняМесяца = Перечисления.ВариантыВыбораДняМесяца.ЧислоМесяца Тогда
			...
			Результат = Дата(Год(ВремДата), Месяц(ВремДата), ПараметрыРебалансировки.ДеньМесяца);
			
		ИначеЕсли ПараметрыРебалансировки.ВариантВыбораДняМесяца = Перечисления.ВариантыВыбораДняМесяца.ДеньНедели Тогда
			
			ВремДата = ДобавитьМесяц(ПредДата, ПараметрыРебалансировки.ЧислоПериодов);
			Если ПараметрыРебалансировки.НомерНедели = 99 Тогда
				// последняя неделя месяца
				ВремДата = КонецМесяца(ВремДата);
				ВремДеньНедели = ДеньНедели(ВремДата);
				Дельта = ?(ВремДеньНедели >= ПараметрыРебалансировки.ДеньНедели, 
							ВремДеньНедели - ПараметрыРебалансировки.ДеньНедели,
							(7 - ПараметрыРебалансировки.ДеньНедели) + ВремДеньНедели);
				Результат = ВремДата - Сутки * Дельта;
			Иначе 
				// n-ая неделя месяца
				ВремДата = НачалоМесяца(ВремДата);
				ВремДеньНедели = ДеньНедели(ВремДата);
				Дельта = ?(ПараметрыРебалансировки.ДеньНедели >= ВремДеньНедели, 
							ПараметрыРебалансировки.ДеньНедели - ВремДеньНедели,
							(7 - ВремДеньНедели) + ПараметрыРебалансировки.ДеньНедели);
				Результат = ВремДата + Сутки * Дельта + 7 * Сутки * (ПараметрыРебалансировки.НомерНедели - 1);
			КонецЕсли; 
...
&НаСервере
Функция ПолучитьПараметрыРебалансировки(СпособРебалансировкиСсылка)
	...
	Запрос.УстановитьПараметр("Ссылка", СпособРебалансировкиСсылка);
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("ВНУТРЕННЯЯ ОШИБКА! Не удалось получить сведения о способе ребалансировки """ + СпособРебалансировкиСсылка + """!");
		Возврат Неопределено;
	КонецЕсли; 
	...
	Возврат ПараметрыРебалансировки;
...
Функция ПолучитьОписаниеПортфеля(ПортфельСсылка)
	...
	ПараметрыРебалансировки = ПолучитьПараметрыРебалансировки(ОписаниеПортфеля.СпособРебалансировки);
	Если ПараметрыРебалансировки = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли; 
	...
	Для Каждого ТекПериод Из МассивПериодов Цикл
		...
		Для Каждого ТекОписаниеПортфеля Из ОписанияПортфелей Цикл
			...
			Если НЕ ПортфельСформирован Тогда
				...
				ТекОписаниеПортфеля.ДатаПоследнейРебалансировки = ТекПериод;
				
				ДобавитьВПротокол("Сформирован " + ПредставлениеПортфеля + " [" + СтрТекПериод + "]: " + ПолучитьСоставПортфеляСтрокой(ТекОписаниеПортфеля));
			...
			ИначеЕсли ЭтоМетапортфель И (АктивныйПортфель <> ТекОписаниеПортфеля.АктивныйПортфель) Тогда
					...
					ДобавитьВПротокол("Переформирован " + ПредставлениеПортфеля + " [" + СтрТекПериод + "]: " + ПолучитьСоставПортфеляСтрокой(ТекОписаниеПортфеля));
			...
			Иначе 
				...
				Если ДатаСледующейРебалансировки <= ТекПериод Тогда
					...
					ДобавитьВПротокол("Перебалансирован " + ПредставлениеПортфеля + " [" + СтрТекПериод + "]: " + ПолучитьСоставПортфеляСтрокой(ТекОписаниеПортфеля));
