...
Процедура ЗаполнитьДанные()
	...
	ДолиВПортфеле = Новый Соответствие;
	Для Каждого СтрокаТабЧасти Из Портфель.СоставПортфеля Цикл
		ДолиВПортфеле.Вставить(СтрокаТабЧасти.Тикер, СтрокаТабЧасти.Доля);
	КонецЦикла; 
	...
	Для Каждого ТекПериод Из МассивПериодов Цикл
		...
		Если ЕстьНезаполненныеКотировки Тогда
			ДобавитьВПротокол("Период " + ТекПериод + " пропущен");
			Продолжить;
		КонецЕсли; 
		
		НоваяСтрока = Данные.Добавить();
		НоваяСтрока.Период = ТекПериод;
		НоваяСтрока.Эталон = ТекущиеКотировки.Получить(ТикерЭталон);
		
		ЗначПортфель = 0;
		Для Каждого КлючИЗначение Из ДолиВПортфеле Цикл
			ТекТикер = КлючИЗначение.Ключ;
			ТекДоля = КлючИЗначение.Значение;
			ЗначПортфель = ЗначПортфель + ТекущиеКотировки.Получить(ТекТикер) * ТекДоля;
		КонецЦикла; 
		НоваяСтрока.Портфель = ЗначПортфель;
...
Процедура РассчитатьСтатистику()
	...
	ПериодНач  = Данные[0].Период;
	ПериодКон  = Данные[КолвоСтрокДанных - 1].Период;
	РазницаВДнях = (НачалоДня(ПериодКон) - НачалоДня(ПериодНач)) / (60 * 60 * 24);
